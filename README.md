...
git clone https://github.com/obieg-zero-dev/plugin-wibor-calc    # to repo
...
```bash
cd plugin-wibor-calc
# edytuj src/index.tsx
cd ../plugins && npm run build              # rebuild index.mjs
cd ../plugin-wibor-calc && git add -A && git commit -m "..."
```
...
```bash
bash core-host/scripts/promote-to-dev.sh  plugin-wibor-calc             # LOCAL -> DEV
bash core-host/scripts/promote-to-prod.sh plugin-wibor-calc v1.2.3      # DEV   -> PROD + tag
```
